...
def enter():
    global genji, throw_knife, stage, menu, temp_bul
    menu = Menu()
    genji = Genji()
    stage = Map()
        ...
        if event.type == SDL_QUIT:
            ...
            if genji.ult_OnOFF == False:
                ...
                if genji.genjistate == Right:
                    throw_knife.append(bullet(genji.x+80, genji.y+60, genji.z, Right))
                else:
                    throw_knife.append(bullet(genji.x-80, genji.y+60, genji.z, Left))

            else:
                genji.ult_attacknum +=1
                genji.ult_flag=1
            ...
            if genji.genjistate == 0:
                genji.x = min(1180,genji.x+350)
            elif genji.genjistate == 1:
                genji.x = max(20,genji.x -350)
            genji.drawnum =0
